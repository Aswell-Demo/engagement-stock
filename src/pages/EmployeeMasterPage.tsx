// src/pages/EmployeeMasterPage.tsx
import { useMemo } from "react";
import dayjs from "dayjs";

type Employee = {
  id: string;
  name: string;
  joinedAt: string;      // 入社日
  registeredAt: string;  // 登録日（システム用）
};

const employees: Employee[] = [
  { id: "100", name: "アズウェル 太郎", joinedAt: "2021/06/01", registeredAt: "2023/06/01" },
  { id: "101", name: "サンプル 次郎", joinedAt: "2023/04/01", registeredAt: "2025/04/01" },
  { id: "102", name: "テスト 花子",   joinedAt: "2025/02/01", registeredAt: "2027/02/01" },
];

const isEligible = (joinedAt: string): boolean => {
  const now = dayjs();
  return dayjs(joinedAt).add(2, "year").isBefore(now);
};

const EmployeeMasterPage = () => {
  const displayData = useMemo(
    () =>
      employees.map((emp) => ({
        ...emp,
        status: isEligible(emp.joinedAt) ? "対象" : "非対象",
      })),
    []
  );

  return (
    <div className="max-w-5xl mx-auto mt-10 bg-white p-6 rounded shadow">
      <h1 className="text-2xl font-bold mb-6 text-center">社員マスタ管理（デモ）</h1>

      <table className="w-full table-auto border">
        <thead className="bg-gray-200">
          <tr>
            <th className="p-2">社員番号</th>
            <th className="p-2">氏名</th>
            <th className="p-2">入社日</th>
            <th className="p-2">登録日</th>
            <th className="p-2">付与対象ステータス</th>
          </tr>
        </thead>
        <tbody>
          {displayData.map((emp) => (
            <tr key={emp.id} className="text-center border-t">
              <td className="p-2">{emp.id}</td>
              <td className="p-2">{emp.name}</td>
              <td className="p-2">{emp.joinedAt}</td>
              <td className="p-2">{emp.registeredAt}</td>
              <td className={`p-2 font-semibold ${emp.status === "対象" ? "text-green-600" : "text-gray-400"}`}>
                {emp.status}
              </td>
            </tr>
          ))}
        </tbody>
      </table>
          <div className="flex justify-end mb-4">
           <button
           className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600"
           onClick={() => window.history.back()}
          >
          戻る
           </button>
          </div>
    </div>
  );
};

export default EmployeeMasterPage;
